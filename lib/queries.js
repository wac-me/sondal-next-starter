...
// ─── Pobierz wszystkie sondy (dla admina) ─────────────────
export async function getAllPolls(supabase, { limit = 50 } = {}) {
...
export async function createPoll(supabase, { question, options, category, isPublic, isAnonymous, authorId }) {
  ...
  const slug = poll.slug || Math.random().toString(36).slice(2, 7);
...
// ─── Pobierz wątki do panelu dyskusji ─────────────────────
export async function getDiscussionThreads(supabase, { limit = 80, authorId = null } = {}) {
  let query = supabase
    .from('comments')
    .select(`
      id, poll_id, author_id, content, created_at, option_id,
      profiles!left ( handle, avatar_letter ),
      polls!inner ( id, slug, question, category, is_public, created_at )
    `)
    .eq('is_hidden', false)
    .eq('polls.is_public', true)
    .order('created_at', { ascending: false })
    .limit(limit);
  ...
  const threadsByPoll = new Map();
  for (const comment of data || []) {
    const poll = comment.polls;
    if (!poll) continue;

    const existing = threadsByPoll.get(poll.id);
    if (existing) {
  ...
  return Array.from(threadsByPoll.values());
}
...
// ─── Sprawdź czy użytkownik ma dostęp do panelu admina ─────
